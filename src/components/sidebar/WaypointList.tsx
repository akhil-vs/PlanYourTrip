...
function SortableWaypoint({
  ...
  return (
    <div
      ref={setNodeRef}
      style={style}
      className="flex items-center gap-2 p-2 rounded-lg border bg-white hover:bg-gray-50 group"
    >
      <button
        {...attributes}
        {...listeners}
        className="cursor-grab active:cursor-grabbing text-gray-400 hover:text-gray-600 min-w-9 min-h-9 flex items-center justify-center touch-manipulation -m-1"
      >
    ...
    >
      ...
      >
        <div className="space-y-2">
          {waypoints.map((wp, i) => (
            <SortableWaypoint key={wp.id} wp={wp} index={i} />
          ))}
        </div>
      </SortableContext>
    </DndContext>
